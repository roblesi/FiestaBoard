...
// All displayable Vestaboard characters (codes 0-71)
...
// Flip tile for loading state - mimics physical Vestaboard flip animation
function FlipTile({ 
  rowIdx, 
  colIdx, 
  size = "md", 
  ...
  const startCharIndex = Math.abs(hash2) % VESTABOARD_CHARS.length;
  ...
  useEffect(() => {
    ...
    const initialTimer = setTimeout(() => {
      setCharIndex((prev) => (prev + 1) % VESTABOARD_CHARS.length);
  ...
  const currentChar = VESTABOARD_CHARS[charIndex];
  const nextChar = VESTABOARD_CHARS[(charIndex + 1) % VESTABOARD_CHARS.length];
